/**
 *   Firmware dos Transmissor utilizado na equipe POTI de futebol de
 *   Robôs da UFRN.
 *   
 *   DCA - Departamento de Computação e Automação / UFRN
 *   30/05/2012
 *
 *   Codigo desenvolvido para ser utilizado com o PIC 18F2550,
 *   bootloader para reprogramação pela USB, e transceiver nRF24L01+.
 *
 *   Codigo inicialmente desenvolvido para MPLAB-Windows e portado
 *   para Linux.
 *
 *   TODO:
 *   - Realizar transmissão individual para cada robô.
 *   - Mudar o pacote para transmitir 11 bits por motor (1 bit de
 *     sentido e 10 de valor)
 * 
 *   TODO Antigos:
 *      Desempacotamento dos dados
 *      Capturar ID
 *	Configuração de Zona Morta
 *
 *   @author Ellon Paiva
 *   @version 1.1
 */
...
/** DEFINES ******************************************************/
//Define as portas de saida que serão utilizadas pelo PIC e seus
//respectivos bits TRIS
...
//Define as portas que serão utilizadas como leds e seus respectivos TRIS.
...
//TODO: Mudar o led que pisca no bootloader
	
//Define os bits que serão lidos para definir ID e FREQ
...
void YourHighPriorityISRCode()
{
  ...
  if(INTCON3bits.INT2IF == 1)//Foi uma interrupção do Transceiver.
    {	
	      
					
      //Lê o status para verificar qual tipo de interrupção foi gerada
      CSN = 0;
      WriteSPI(R_REGISTER | STATUS);
      my_status = ReadSPI();
      CSN = 1;
      if((my_status & 0b00010000) != 0){ //Verifica se a interrupção foi devido ao máximo de retransmissões.
	PORTAbits.RA1 = 1; //pisca o led RA1
	//2) Limpa o bit de interrupção
	...
	CSN = 1;
      ...
      if((my_status & 0b00100000) != 0){ //Verifica se a interrupção foi devido a transmissão do pacote~;.
	//2) Limpa o bit de interrupção
	my_status = my_status | (1<<TX_DS);
	CSN = 0;
	WriteSPI(W_REGISTER | STATUS);
	WriteSPI(my_status);
	CSN = 1;
      ...
      INTCON3bits.INT2IF = 0; //limpa o flag de interrupção externa 2 (trans)
...
void UserInit(void)
{
   ...
   **************************************************************************/
  //Define as entradas das chaves que configuram a FREQ.
  SWCH_FREQ0_TRIS = 1;
  SWCH_FREQ1_TRIS = 1;
  ...
  //Definindo porta para led's como saidas.
  LED1_TRIS = 0;
  LED2_TRIS = 0; 
  LED3_TRIS = 0; 
  LED4_TRIS = 0; 
  ...
  //Habilita interrupção INT2, que fica no pino RB2.
  ...
  //Inicializa todas as posicoes do vetor de recepção e transmissão da USB com '0'
  for(i=0; i<64; i++){
  ...
  /**************************************************************************
   * Lê os dados das chaves para setar o canal de transmissão.
   **************************************************************************/
  //Lê os bits que definem a frequencia a ser utilizada das
  ...
  //|_CAB_|_D0_|_E0_|_D1_|_E1_|_D2_|_E2_|_FQ_|
	
  //Realiza um delay de 150ms, que é o tempo necessário para fazer a
  //inicialização do transceiver. Não se sabe ao certo quanto tempo já
  //se passou desde o início da execução do código, mas como o tempo
  //de inicialização é 100ms, o delay aqui aplicado é >100ms.
  Delay10KTCYx(180); //Pausa de 150ms
	  
  //Coloca o transmissor no modo Power Down. Esse processo é
  //necessário no caso do PIC estar sendo reiniciado após um reset,
  //pois o transceiver já teria sido configurado e estaria em modo
  //Power Up e os comandos de configuração não funcionariam. No caso
  //do pic estar sendo configurado a primeira vez após ser alimentado,
  //esse comando não causa problemas.
  CSN = 0;
  WriteSPI(W_REGISTER | CONFIG); //Comando de escrita em registrador combinado com endereço do registrador
  WriteSPI(STARTUP_CONFIG | (0<<PWR_UP));
  CSN = 1;
	
  //Configura taxa de transmissão e potencia do transceiver 
  CSN = 0;
  ...
  //variavel 'channel'. Ela pode ser 2410MHz, 2420MHz, 2430MHz ou 2440MHz.
  CSN = 0;
  WriteSPI(W_REGISTER | RF_CH); 
  WriteSPI(channel);
  CSN = 1;
	
  //Muda o endereço do transmissor para o endereço utilizado pelos robôs. Como atualmente é enviado somente um pacote, o endereço dos robôs é o mesmo, e logo o endereço do transmissor precisa ser setado somente uma vez. 
  //Caso seja implementado a transmissão de um pacote por robô, deve-se setar o endereço ANTES de enviar cada pacote, na função ProcessIO(), e não no UserInit().
  CSN = 0;
  WriteSPI(W_REGISTER | TX_ADDR);
  //Todos os endereços contidos na variavel 'addr_id_x[]' são iguais
  //quando é enviado apenas um pacote para os três robôs.
  for(i=0;i<5;i++){
    WriteSPI(addr_id_0[i]); 
  }
  CSN = 1;
	
  //Muda o endereço do RX_ADDR_P0 do transmissor para o endereço do
  //robô a ser transmitido o pacote. Isso é necessário para receber o
  //ACK, que sempre é enviado no canal 0. Tecnicamente não seria
  //necessário, pois ao transmitir um pacote para 3 robôs o ACK deve
  //ser desabilitado, mas por completude, seta-se o registrador assim
  //mesmo.
  CSN = 0;
  ...
  //Habilita o ACK dinamico, que permite enviar um pacote sem esperar
  //receber um ack após a transmissão. Necessário ao transmitir para
  //um mesmo pacote para 3 robôs. Pode ser removido caso mude para uma
  //transmissão de um pacote por robô.
  CSN = 0;
  WriteSPI(W_REGISTER | FEATURE); 
  WriteSPI(1<<EN_DYN_ACK); 
  CSN = 1;
	
  //Coloca o transceiver em modo Power Up, setando ele como
  //transmissor. É recomendado realizar uma pequena pausa após esse
  //comando para esperar o oscilador do transceiver ser inicializado.
  CSN = 0;
  WriteSPI(W_REGISTER | CONFIG); 
  WriteSPI(STARTUP_CONFIG | ((1<<PWR_UP) | (0<<PRIM_RX))); 
  CSN = 1;
		
  //Realiza uma pausa de 200us, que é um pouco maior que o tempo de
  //start up do oscilador até o transceiver entrar no modo Standby-I.
  Delay100TCYx(24); //Delay de 200us
	  
  //Esvaziando buffer de recepção
  CSN = 0;
  WriteSPI(FLUSH_RX); 
  CSN = 1;
			
  //Esvaziando buffer de transmissão
  CSN = 0;
  WriteSPI(FLUSH_TX); 
  CSN = 1;
...
{   
  ...
  //TODO: Descobrir o que o comando abaixo faz.
  // User Application USB tasks
  if((USBDeviceState < CONFIGURED_STATE)||(USBSuspendControl==1)) return;
	    
  //Recebe um novo dado para transmitir.
  ...
  //Verifica se tem dado a ser transmitido.
  if(newDataToSend == 1){
	    
    //Coloca o pacote a ser transmitido no buffer de transmissão. Ao
    //utilizar o comando W_TX_PAYLOAD_NO_ACK o transmissor não vai
    //esperar por um ACK após transmitir o pacote. Necessário ao
    //transmitir um mesmo pacote para 3 robôs. Deve ser mudado para
    //W_TX_PAYLOAD caso seja implementado a transmissão de 1 pacote
    //por robô.
    CSN = 0;
    WriteSPI(W_TX_PAYLOAD_NO_ACK);
    ...
    //Transmite o dado, setando o pino CE. Para transmitir, o valor de
    //CE precisa ficar setado por pelo menos 10us. Por segurança,
    //mantem-se CE setado pelo dobro desse valor, no caso 20us.
    CE = 1;
    Delay10TCYx(24); //Delay de 20us
    CE = 0;
	  
    //OBS: Caso seja implementado o envio de 1 pacote por robô, é
    //necessário setar os registradores TX_ADDR e RX_ADDR_PO do
    //transmissor para o mesmo endereço do robô receptor ANTES de
    //transmitir cada pacote. Os endereços utilizados estão
    //armazenados na variavel addr_id_x, onde 'x' é o ID do respectivo
    //robô. Atualmente esses endereços são setados uma vez, na
    //inicialização.
	
    //TODO: Analizar se é melhor deixar o transmitor em Standby-II ou
    //Standby-I. (CE = 1 o tempo todo ou pulsar quando tiver com o
    //pacote inteiro). A duvida surgiu porque sempre que tem novos
    //dados no buffer ele envia automaticamente. Ou seja, cada byte
    //adicionado é enviado para o receptor. O que acontece caso haja
    //falhas na transmissão de um byte enquanto 2 são adicionados na
    //fila de transmissão? Eventualmente o pacote seria transmitido em
    //uma ordem errada. Ex: quer se enviar 1, 2, 3 e 4, mas acaba-se
    //enviando 1, 3, 2, 4, pq ao transmitir 1 teve um atraso, dai
    //foram colocados 3 e 2 no buffer e como ele envia o LSByte
    //primeiro, o 3 foi enviado antes do 2.
	
	
    //Pisca um led para informar que foi transmitido.
    LED5 ^= 1;
	
    //Como o dado foi transmitido, reseta o flag.
    newDataToSend = 0;
  }
  //    ReceivedDataBuffer[0] = 0x80;
  //   	ReceivedDataBuffer[13] = 0x7F;
  //	CSN = 0;
  //    WriteSPI(W_TX_PAYLOAD_NO_ACK);
  //    for(i = 0; i<payload; i++){
  //      WriteSPI(ReceivedDataBuffer[i]);			
