...
/* Este programa tem uma thread principal que espera que o usuário
   digite alguma coisa e envia esta string para o seu correspondente
   via socket. Uma outra thread imprime todas as strings recebidas
   pelo socket. Para toda mensagem recebida ou enviada, inicialmente é
...
int main(void)
{
  ...
  if (s.listen(PORTA_TESTE) != SOCKET_OK) {
    cout << "Problema na abertura do socket...\n";
    exit(1);
  }
  if (s.accept() != SOCKET_OK) {
